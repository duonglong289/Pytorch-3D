...
def step(split, epoch, opt, dataLoader, model, criterion, optimizer = None):
  ...
  for i, (input, target2D, target3D, meta) in enumerate(dataLoader):
    ...
    # print(target3D_var)
    if opt.DEBUG >= 2:
      gt = getPreds(target2D.numpy()) * 4
      pred = getPreds((output[opt.nStack - 1].data).numpy()) * 4
      debugger = Debugger()
    ...
    Acc.update(Accuracy((output[opt.nStack - 1].data).cpu().numpy(), (target2D_var.data).numpy()))
    mpjpe, num3D = MPJPE((output[opt.nStack - 1].data).cpu().numpy(), (reg.data).numpy(), meta)
    if num3D > 0:
